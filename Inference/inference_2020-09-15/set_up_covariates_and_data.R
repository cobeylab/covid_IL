...
population_list=read.csv(covid_get_path(population11_file)) %>%
    mutate(region = aggregated_regions[covid_region]) %>%
    group_by(region, AGE_GROUP_MIN) %>%
...
civis_data = read.csv(covid_region_data_filename)  %>% 
    mutate(Date=as.Date(date), 
        ...
        restore_region = aggregated_regions[covid_region]) %>%
    select(-covid_region, -date, -source_hosp_deaths) %>%
...
idph = read.csv(idph_filename) %>% 
    mutate(Date=as.Date(date), region=aggregated_regions[new_restore_region]) %>%
...
df = civis_data %>% mutate(
    ...
    mutate(date=Date) %>%
    ungroup() %>%
    select(restore_region, confirmed_covid_icu, covid_non_icu, total_deaths, emr_deaths, nonhosp_deaths, new_ll_hospitalizations, date, source) %>%
    mutate(confirmed_covid_icu = if_else(confirmed_covid_icu < 0, 0, round(confirmed_covid_icu)),
           ...
           confirmed_covid_nonicu = if_else(covid_non_icu < 0, 0, round(covid_non_icu)),
           emr_deaths = if_else(emr_deaths < 0, 0, round(emr_deaths)),
           nonhosp_deaths = if_else(nonhosp_deaths < 0,0,round(nonhosp_deaths)),
           new_ll_hospitalizations = if_else(new_ll_hospitalizations < 0,0,round(new_ll_hospitalizations)))
...
data[1, names(data %>% select(-time))] = NA
...
if (use_changepoint){
    ...
    log=c(paste0('beta1_', 1:pars$n_regions),
        paste0('beta2_', 1:pars$n_regions),
        paste0('scale_phase3_', 1:pars$n_regions),
        paste0('scale_phase4_', 1:pars$n_regions),
        paste0('scale_phase5_', 1:pars$n_regions)),
    logit=paste0('num_init_', 1:pars$n_regions)
    )
}
