...
simulate_pomp_covid__init_parameters <- function(
  ...
  with(input_params, {
    ...
    params[paste0("num_init_",c(1:n_regions))] = unlist(input_params[paste0('num_init_', c(1:n_regions))])
    ...
    if (use_changepoint){ # Only add these parameters if you're using a changepoint model
      params[paste0("beta2_",c(1:n_regions))] = unlist(input_params[paste0('beta2_', c(1:n_regions))])
    ...
    # age and region
    for(i in c(1:n_regions)){
      params[paste0(paste0("N_",c(1:n_age_groups)),"_",i)] = population_list %>% 
        filter(region == i) %>% 
        select(POPULATION) %>% 
        unlist(use.names=F)
    }
