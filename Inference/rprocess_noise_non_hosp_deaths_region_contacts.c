...
const double *phi = &phi_1; // phi is fraction mild cases that recover 
...
for (int region=0; region<n_regions; region +=1)
{
    ...
    for (int j=0; j<num_age_groups; j += 1){
      ...
      double dP_m_to_recover = rbinom(dP_m, phi[j]); // phi is fraction mild cases that recover
      double dP_m_to_death = rbinom(dP_m, (1-phi[j]));
