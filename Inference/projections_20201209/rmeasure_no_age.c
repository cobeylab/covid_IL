...
for (int region=start_loop; region<end_loop; region += 1){
    ...
    //ObsDeaths[region] = rbinom(agg_new_D, total_death_reporting); 
    //Hospital deaths
    ...
    //ObsHospDeaths[region] = rbinom(agg_new_D, hosp_death_reporting); 
    ...
    //ObsHosp[region] = rbinom(agg_new_hosp, total_hospital_reporting); 
    ...
    if (ISNA(icu_reporting) & t > 200){
        icu_reporting = 1;
    }
    ObsICU[region] = rpois(agg_new_icu * icu_reporting);
    //ObsICU[region] = rpois(agg_new_icu, icu_reporting); 
}