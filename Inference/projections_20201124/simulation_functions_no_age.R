...
simulate_pomp_covid__init_subcompartment_df <- function(input_params) {
  with(input_params, {
    data.frame(
      state = c("E","P","IM","IM_dead","IS", "IH1_", "IH2_"),
      index = c(
        alpha_E,
        alpha_P,
        alpha_IM, alpha_IM,
...
simulate_pomp_covid__init_state_names <- function(n_age_groups, n_regions, subcompartment_df) {
  ...
  state_names <- c(state_names_S, state_names_R, state_names_D, state_names_trans, state_names_hfr, state_names_phi, state_names_ihr, state_names_ifr)
...
simulate_pomp_covid__init_covariate_table <- function(input_params, 
  ...
  if (!is.null(beta_covar)){
    covar_table_interventions = left_join(covar_table_interventions, beta_covar)
  }

  # add beta scaling factors
  if (!is.null(beta_scale_covar)){
    covar_table_interventions = left_join(covar_table_interventions, beta_scale_covar, by='time')
  }
