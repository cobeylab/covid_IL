...
ll_deaths = read.csv(covid_get_path(ll_file)) %>%
  ...
  mutate(Compartment = 'Reported deaths') %>%
  mutate(Source='Linelist')

cumulative_ll = ll_deaths %>%
  arrange(Date) %>%
  mutate(Cases = cumsum(replace_na(Cases, 0)),
...
sim_0 = 0
slice_df = read.csv(sprintf("%s/scenario_num_sims_%s_%s.csv", output_dir, model_name, region_to_test)) %>%
  filter(replicates > 0) %>%
...
foreach (r=1:nrow(slice_df), .combine='rbind') %do%{
  ...
  n_sim_project = ceiling(slice_df[r, 'replicates'])
  ...
  sim_temp = simulate_pomp_covid(
    ...
    obsnames = c(paste0('ObsHosp_', 1), paste0('ObsDeaths_', 1), paste0('ObsICU_', 1)),
    use.mle.params = T,
    mle.params=new_pars
  )
  sim_temp$raw_simulation_output = sim_temp$raw_simulation_output %>%
    mutate(.id = as.numeric(.id) + sim_0)
...
# Make basic plots
compartments_to_plot = c('ObsDeaths', "ObsHosp", "ObsICU", 'Inc', "Total infectious",  "R", 'new_hospitalizations', 
      'transmissionRate', 'HFRtrack','IHRtrack', 'freqtrack')
new_compartment_name_map = c('Reported deaths', 'Reported census hospital beds occupied', 'Census ICU beds occupied',
  'Daily new infections', 'Total infectious', 'Fraction recovered', 'new_hospitalizations', 'Transmission rate',
  'HFR','IHR', 'Variant frequency')
names(new_compartment_name_map) = compartments_to_plot
plot_order = c('Reported deaths','Reported cumulative deaths', 'Reported census hospital beds occupied', 'Census ICU beds occupied', 'new_hospitalizations', 
