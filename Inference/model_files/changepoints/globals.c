...
void initialize_state(int reg, int alpha_int, double* State){
    int Start = reg * alpha_int;
    for (int j=0; j<alpha_int; j++){
      int offset = Start + j; 
      State[offset] = 0;
    }
}

double move_classes (double* State, double* newState, int alpha_int, double freq, int reg){
  int Start = reg * alpha_int; 
  for (int j=0; j<alpha_int; j++){
    int offset = Start + j;
    double move = rbinom(State[offset], freq); 
    State[offset] += -move;
    newState[offset] += move;
  }
