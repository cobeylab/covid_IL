...
// Tracking states for debugging
    ...
    double *transmissionMean = &transmissionMean_1_1; 
    double *HFRtrack = &HFRtrack_1_1;
    double *HFRMean = &HFRMean_1_1;
    double *ICUtrack = &ICUtrack_1_1;
    double *ICUMean = &ICUMean_1_1;
...
for (int region=start_loop; region<end_loop; region += 1){
    ...
    double HFR = get_HFR(hfr_changepoint_offset, n_hfr_changepoint_int, t);
    HFRMean[region] = HFR;
    HFR += HFR * process_noise;
    HFRtrack[region] = HFR;

    // Figure out fraction going to the ICU
    process_noise = sqrt(dt) * eta * rnorm(0,1);
    ...
    ICUMean[region] = ICUfrac;
    ICUfrac += ICUfrac * process_noise;
    ICUtrack[region] = ICUfrac;
    ...
    // Figure out total infectious
      ...
      transmissionMean[region] = transmission;
