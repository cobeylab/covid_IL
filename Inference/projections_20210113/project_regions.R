...
if (simulate_new == T)
{
  ...
  foreach (r=1:nrow(slice_df), .combine='rbind') %do%{
    ...
    for (p in names(slice_df)){
      pars[[p]] = slice_df[r, p]
    }
    print(pars[['n_changepoints_10']])
  ...
  plotting_output = alloutputs$plotting_output %>%  
      filter(Region == region_to_test) %>%
      mutate(Date=as.Date('2020-01-14') + Time)
} else{
  projections = read.csv(sprintf("%s/projections_%s_%s.csv", human_output_dir, model_name, region_to_test)) %>%
    mutate(date = as.Date(date))
  print('Made projections')
  plotting_output = projections %>% 
    rename(Date = date) %>%
...
ggsave(sprintf('%s/projection_%s_%s.png', human_output_dir, model_name, region_to_test), width=14, height=14)
