...
last_date = as.Date('2021-03-28')
first_date = as.Date('2020-03-01')
last_plot_date = last_date + 30
fit_results = './latest_fit_results/uchicago_github_20210331.csv'
cdc_hosp_file = '../Data/COVID-19_Reported_Patient_Impact_and_Hospital_Capacity_by_State_Timeseries.csv'
...
public_data = read.csv('../Data/idph_public_covid_region.csv') %>%
    ...
    filter(date >= first_date, date<=as.Date(last_date)) %>%
...
foreach(r=1:11, .combine='rbind') %do%{
    read.csv(sprintf("model_rt_estimates/model_rt_%s_%s.csv", r, '2021-03-31')) %>%
        mutate(date =as.Date(date))
} -> rt_model
